...
- **Async job pipeline** — jobs are processed in the background; poll for status or subscribe to email notification
...
```bash
cp backend/.env.example backend/.env   # or create the file manually (see below)
```
...
See [LOCAL_SETUP.md](LOCAL_SETUP.md) for a step-by-step Windows guide. General steps:
...
1. **Ollama** (local, zero rate limits) — set `OLLAMA_URL` and pull a model, e.g.:
   ```bash
   ollama pull qwen2.5:3b
   ```
2. **OpenRouter free models** — set `OPENROUTER_API_KEY`; Sneak tries `google/gemma-3-4b-it:free` and others automatically
3. **LocalSummarizer** — pure keyword extraction, no API key needed, always works
